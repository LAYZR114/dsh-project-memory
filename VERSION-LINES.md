...
- 2026-09-11：**「选择目录」按钮 + 历史污染修正**（用户需求/反馈）：①输入框右侧新增「选择目录」，走官方宿主 seam `ctx.directoryPicker.capability()` —— `kind:'native'` 时 `pick(signal)` 弹**系统原生文件夹对话框**（桌面端），选中即切项目并加载；取消 → `cancelled`（静默）；只有 `browse` 能力时如实上报 `browse-only`（不假装成功）；经新增 API 动作 `action:'pickdir'` 暴露。②**修历史污染**（用户实测：下拉被 `D:/D`、`D:/DS` 灌满）：去掉"输入即自动加载"（`useEffect` 只挂载跑一次）、`load()` 不再记历史（只有回车/保存/选择目录/下拉选择这些**明确提交**才记）、`collectProjects()` **只列真有记忆文件的目录**、新增纯函数 `cleanRecentDirs()` 清理旧的半截路径。`test/dir-typeahead.test.mjs` 扩到 19 项，全量 **93/93** 通过。
