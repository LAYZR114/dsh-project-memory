...
- 2026-09-11：**项目目录 Typeahead**（用户需求）：输入框**回车即加载**该目录记忆；聚焦弹**可搜索下拉**列出"现有项目"——宿主新增 `collectProjects()`（本进程见过的 cwd（`noteCwd` 由 `ensureCwd`/`readDoc` 记录）+ 活跃会话 `agents.list()` 的工作目录，每条附 `count` 记忆条数与 `updatedAt` 文件时间），经新增 API 动作 `action:'projects'` 暴露；客户端合并本地"最近使用"（localStorage），输入即过滤、↑↓/鼠标选择、Enter/Esc，选中即切换项目。新增 `test/dir-typeahead.test.mjs`（9 项：聚合真单测 4 + 真 api 路由 2 + 接线 3），全量 **84/84** 通过。
