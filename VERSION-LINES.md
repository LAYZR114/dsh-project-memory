...
- 2026-09-11：**「选择目录」改用更普适的官方入口 + 按钮尺寸统一**（用户实测：桌面端提示"没有系统文件夹对话框"）：桌面端实现的是 **browse 式**目录选择（没有系统原生 `pick`），所以改为**三级优先**：① **客户端服务 `uiWorkspace.pickDirectory()`**（`ctx.get('uiWorkspace')` 软依赖）——即 DSH 应用自带"选文件夹"对话框（面包屑/新建文件夹），桌面端可用；② 退回宿主 `ctx.directoryPicker` 的 `native pick()`（经 `action:'pickdir'`）；③ 都没有则如实提示改手输/下拉。取消一律静默。按钮改为 `pm-save pm-pick`（**与「保存」同 padding/字号**，只多一个 Feather 文件夹图标）——用户要求：以后做按钮尽量减少尺寸/风格差异。
