...
- 2026-09-11：**统一优先级：拖动排序 = 「保存排序」**（用户定调）。两者本就同一条写盘路径（`persist → /save`），但拖动那条**不看服务端回读校验**，可能"界面是新顺序、文件是旧顺序"。现在拖动提交也走 `verified` 门槛：失败/回读不一致 → 如实提示 + 按磁盘真实顺序重载界面（绝不谎报成功），成功后挂 **10 秒撤销**并提示"顺序已保存（拖动即保存）"。手动（默认）模式渲染的就是文件真实顺序 → **重启后永远是你最后摆的样子**。新增接线回归 `test/client-drag-save-wiring.test.mjs`（5 项）。全量 **62/62** 通过。
