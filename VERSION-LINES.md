...
- 2026-09-11：**新增导入向导**（用户需求）：设置页「导入」选好 JSON 后**先弹确认框**（不再直接落盘）——插件对每条自动判断**分类**（关键词打分；合法显式 type 优先）与**重要等级**（义务语义「必须/禁止/每次…」→ 长期有效 + obligation，否则沿用原 ttl），按**原始正文首行**生成标题/简介（≤40/60 字，剥掉 "1、/- " 前缀；正文用 `organizedText` 自动排版），逐条给出分类/等级下拉与可编辑标题、简介；点「确定导入」才写盘，成功后**置顶**（分类分组后即显示在各组顶部），id 与现有记忆或本批内重复时自动换新；失败/回读不一致按磁盘顺序重载界面。新增 `test/import-classify.test.mjs`（12 项：判定真单测 10 + 接线 2），全量 **75/75** 通过。
