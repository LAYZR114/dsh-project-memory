...
import { existsSync, readFileSync, statSync } from 'node:fs'  // 首步真身：context 内同步读盘（DSH fs 服务纯异步，宿主插件可用 node:fs——hooks-codex 先例）
...
// ---- readMemoryFileMtime：记忆文件最后修改时间（ms）；不存在/读不到 → 0。只读，供设置页「现有项目」排序 ----
export function readMemoryFileMtime(cwd, MEMORY_FILE_ = '.dsh-memory.json') {
  try {
    const p = `${cwd}${cwd.endsWith('\\') || cwd.endsWith('/') ? '' : '\\'}${MEMORY_FILE_}`
