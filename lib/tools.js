          ...
          try {
            ...
            if (route && route.provider && route.model) {
              ...
              for await (const chunk of llm.stream({ provider: route.provider, model: route.model, messages, maxTokens: 900, purpose: 'session-title' })) {
                ...
                // 文本增量：dsh-llm 的 StreamChunk 只有 'text-delta'（0.1.2/0.1.5 逐字符相同，源码 packages/llm/llm/src/types.ts）；
                // 旧代码判 'content'/'text' 永不命中 → raw 恒空 → JSON.parse 必抛 → 静默回退词法排序（LLM 精排从未生效）。
                // 保留旧名容错，主判据是 'text-delta'。
                if (chunk.type === 'text-delta' || chunk.type === 'content' || chunk.type === 'text') raw += (chunk.text || '')
              }
