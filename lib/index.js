...
export function apply(ctx, config) {
   ...
   */
  /**
   * 弹出系统"选择文件夹"对话框（设置页「选择目录」按钮用）。
   * 走官方宿主 seam `ctx.directoryPicker`（抽象服务）→ `capability()`：
   *  · kind==='native' → `pick(signal)` 返回绝对路径（桌面端 Electron 原生对话框）
   *  · kind==='browse' → 只有浏览能力（无原生对话框）→ 如实上报，客户端可退回手输/下拉
  ...
  async function pickDirectory() {
    ...
    let cap
    try { cap = svc.capability() } catch (e) {
