...
export function apply(ctx, config) {
  ...
  async function writeThrough(cwd, memories) {
    const bad = checkSanity(memories)
    if (bad) { report('writeThrough ABORTED: ' + bad); return false }
    const ok = await storeWriteDoc(cwd, memories)
    if (!ok) { report('writeThrough storeWriteDoc failed (fs unavailable?)'); return false }
    await afterWrite(cwd, memories)
    return true
  }
