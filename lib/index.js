...
export function apply(ctx, config) {
  ...
  /**
   * 弹系统"选择文件夹"对话框（「选择目录」按钮）。
   * 三级：① 宿主 remote 控制器 `directoryPickerController.pick()`（**驱动应用自带的系统文件夹对话框**，
   * 就是截图里那种 Windows 文件管理器对话框，最快）；② `capability()` 为 `native` 时 `pick()`；
   * ③ 都没有 → `browse-only`（客户端改用自己的浏览对话框兜底）。
   * @returns {Promise<{ok:true,cwd:string}|{ok:false,error:string,browse?:boolean}>}
   */
  async function pickDirectory() {
    // ① 应用自带的系统对话框（由客户端 UI 呈现；桌面端实测可用）
    try {
      const ctl = ctx.get('directoryPickerController')
      if (ctl !== undefined && typeof ctl.pick === 'function') {
        const p = await ctl.pick(AbortSignal.timeout(300000))
        if (typeof p === 'string' && p) { noteCwd(p); return { ok: true, cwd: p } }
        if (p === null) return { ok: false, error: 'cancelled' } // 用户取消
      }
    } catch (e) { /* 控制器不可用/该环境没连客户端 → 落到下一级 */ }
    // ② 宿主 provider 的原生对话框（有系统 pick 能力的环境）
    const got = directoryPickerCapability()
    if (!got.ok) return got
    const cap = got.cap
    if (cap.kind !== 'native' || typeof cap.pick !== 'function') {
      return { ok: false, error: 'browse-only', browse: true } // 如实上报，不假装成功
    }
    try {
