    ...
    try {
      ...
      const handler = async (req, res) => {
        ...
        try {
          ...
          if (action === 'browse') {
            // 设置页自带"选择文件夹"浏览对话框：列出 path 下的子目录（官方 browse 能力）。
            // 返回 { ok:true, listing:{ path, home, crumbs, entries, truncated } } 或 { ok:false, error }。
            const got = (typeof browseDirectories === 'function')
              ? await browseDirectories(payload.path)
              : { ok: false, error: 'browse unavailable' }
            return json(res, got)
          }
          if (action === 'projects') {
