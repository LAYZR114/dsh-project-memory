...
/* 搜索：**按钮常驻**，点它展开/收起输入框（用户要求：按钮不消失、再点收起）。
   展开的输入框用 width 过渡从 0 展开，靠 flex:0 1 Npx 主动让位，**绝不换行/撑宽整行**（防左右滚动条）。 */
.pm-search-btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;background:#1f1f1f;color:#b5b5b5;border:1px solid #2c2c2c;border-radius:20px;padding:5px 12px;cursor:pointer;flex:0 0 auto;height:30px;box-sizing:border-box}
.pm-search-btn:hover{background:#262626}
.pm-search-btn.on{background:#3a3a3a;color:#f0f0f0;border-color:#3a3a3a}
.pm-search-box{display:inline-flex;align-items:center;gap:6px;background:#1a1a1a;border:1px solid #333;border-radius:20px;padding:0 12px;height:30px;box-sizing:border-box;flex:0 1 180px;min-width:0;overflow:hidden;animation:pm-search-in .18s ease}
.pm-search-box input{flex:1;min-width:0;background:transparent;border:none;color:#e6e6e6;font-size:12px;outline:none;padding:0}
...
@keyframes pm-search-in{from{opacity:0;transform:scaleX(.9);transform-origin:left}to{opacity:1;transform:scaleX(1)}}
		...
		function MemorySection() {
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-bar" },
					...
					// 搜索（用户要求）：**按钮常驻**，点一下展开、再点收起；有词时按钮带小徽标显示当前词。
					// 展开的输入框是紧挨按钮的独立小盒子（宽度过渡展开，flex 让位）——**不换行、不撑宽整行**。
					h("button", {
						key: "search-btn",
						className: "pm-search-btn" + ((searchOpen || q) ? " on" : ""),
						onClick: () => setSearchOpen((v) => !v),
						title: searchOpen ? "收起搜索" : "搜索记忆",
					}, [
						h("svg", {
						...
						q ? h("span", { key: "badge", className: "pm-badge", title: q }, q) : null,
					]),
					searchOpen ? h("div", { key: "search-box", className: "pm-search-box" }, [
						h("input", {
