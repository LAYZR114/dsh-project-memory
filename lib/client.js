...
/* 目录行里的胶囊吸走行尾空白 → 保证「输入框/保存/🔍」三者紧贴左侧（用户要求：收起态必须紧贴） */
.pm-toolbar .pm-stat{margin-left:auto}
/* 展开搜索时：输入框主动缩窄让位（键在 110px），🔍 因此**位置不变**，搜索框只向🔍右侧长出 */
.pm-toolbar.pm-toolbar-search .pm-ac-wrap{width:150px}
...
/* —— 目录行（用户定案：**四个控件紧贴、行尾留白归胶囊**）——
   ...
   三条硬约束（用户原话）：
     ...
     ② 搜索框**只向🔍的右边**展开（方向向右）；
     ③ 展开过程中**🔍 的 x 坐标保持不变** → 输入框吸收宽度变化（它是唯一可压缩项），
        且搜索框在🔍之后，因此🔍不会被推走。
...
/* 输入框：**不抢剩余空间**（不写 flex:1），只在自己那块宽度里伸缩；展开搜索时它主动缩窄让位 */
.pm-toolbar .pm-ac-wrap{flex:0 1 auto;width:240px;min-width:120px}
...
/* 搜索：图标按钮常驻；展开时输入框**向右推开**（宽度过渡），右侧「记忆 N」自然让位（零遮挡）。
   ...
   ② 展开 210px（过渡 220ms ease-out；微交互 100–200ms、显隐 200–300ms，不用 ease-in）；
...
.pm-search-slot.open{width:210px}
.pm-search-inner{width:210px;display:flex;align-items:center}
.pm-search-box{display:inline-flex;align-items:center;gap:6px;height:36px;width:210px;background:#141414;border:1px solid #141414;border-left:none;border-radius:0 10px 10px 0;padding:0 10px 0 8px;box-sizing:border-box}
...
`;
		...
		function MemorySection() {
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-toolbar" + (searchOpen ? " pm-toolbar-search" : "") },
							...
							h("div", { className: "pm-search-inner" },
								h("div", { className: "pm-search-box" }, [
									h("input", {
										...
										// ⚠️ 绝不挂 onBlur 自动收起：点按钮本身会让输入框失焦 → 刚展开就被收起，用户根本没法打字（实测）。
										// 收起只由「再点按钮」或「Esc 且词为空」触发；点别处一律保持展开。
