...
/* 目录行里的胶囊：**紧跟🔍/搜索框**（不再用 margin-left:auto 吸走空白飞到最后）。
   用户要求：收起时胶囊紧贴🔍；展开时被搜索框向右推开 100px（让位），但**始终留在可视区内**。
   行尾多余空间**留白**（不分配给任何控件）→ 胶囊永远不会被挤出屏幕。 */
.pm-toolbar .pm-stat{margin-left:0}
/* 搜索展开时：胶囊缩成"只显示数字"（省下"记忆"两字与内边距 ≈48px）。
   解决"面板 ≈540px 装不下展开态 612px、胶囊被顶出屏幕"（用户定案）。
   ⚠️ 必须写成 .pm-stat.pm-stat-mini b（特异性 0,2,1）才能压过后面的 .pm-stat b（0,1,1）；
   只写 .pm-stat-mini b 会因"同特异性、后者胜"而失效 → 数字被 margin-left 推偏、不居中（用户实测）。 */
.pm-stat.pm-stat-mini{padding:0 10px;justify-content:center}
.pm-stat.pm-stat-mini b{margin-left:0;font-size:13px;line-height:1;color:#e6e6e6}
/* ⚠️ 展开搜索时**输入框宽度必须保持不变**（170px）。
   曾经写成缩到 100px「让位」——但输入框在🔍**左边**，缩窄会把它右边的
...
/* 搜索：图标按钮常驻；展开时输入框**向右推开**（宽度过渡），右侧「记忆 N」自然让位（零遮挡）。
   ...
   ② 展开 100px（过渡 220ms ease-out；微交互 100–200ms、显隐 200–300ms，不用 ease-in）；
...
.pm-search-slot.open{width:100px}
.pm-search-inner{width:100px;display:flex;align-items:center}
.pm-search-box{display:inline-flex;align-items:center;gap:6px;height:36px;width:100px;background:#141414;border:1px solid #141414;border-left:none;border-radius:0 10px 10px 0;padding:0 10px 0 8px;box-sizing:border-box}
...
`;
		...
		function MemorySection() {
			...
			// 记忆搜索框的 DOM 引用（唯一聚焦入口是 useEffect；元素仅在展开时存在）
			...
			// 记忆搜索框的聚焦：**全场唯一的聚焦入口**。
			// 结构性前提：input 只在 searchOpen=true 时才存在于 DOM（收起=卸载）。
			// 因此这里的聚焦**不可能影响目录输入框**：只有"刚展开"那一刻，元素才刚被创建。
			// 逻辑：
			//   · 依赖数组**必须且只能是 [searchOpen]**；
			//   · 用 requestAnimationFrame 等 DOM 挂载完成后再聚焦（比魔数 setTimeout 更准）；
			//   · 若用户此刻正在别的输入框打字，则**不抢**（避免打断）。
			// ⚠️⚠️ 绝不可把 q 或其它状态加入下面的依赖数组！
			//   一旦加入，每次输入都会重跑本 effect → 每次都 focus() → "抢焦点"问题会第三次复发
			//   （该问题已因 autoFocus、回调 ref 分别复发过两次）。
			useEffect(() => {
				if (!searchOpen) return undefined;
				// 聚焦动作只写一份（避免重复代码）；调度方式优先 rAF，缺则退回 setTimeout
				const focusNow = () => {
					...
					try { el.focus(); moveCaretToEnd(el) } catch (e) { /* 忽略 */ }
				};
				const hasRaf = typeof requestAnimationFrame === "function";
				const handle = hasRaf ? requestAnimationFrame(focusNow) : setTimeout(focusNow, 60);
				return () => { if (hasRaf) cancelAnimationFrame(handle); else clearTimeout(handle); };
			}, [searchOpen]);
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-toolbar" },
					...
					h("div", { key: "search", className: "pm-search" }, [
						...
						// 搜索框：**收起时整个从 DOM 卸载**（结构性根治"抢焦点"）。
						// 历史教训（同一问题复发两次）：
						//   ① input 的 autoFocus —— 每次重渲染都执行 → 抢走目录输入框的焦点；
						//   ② 回调 ref 里调 focusEnd()（内含 focus()）—— 回调 ref 每次渲染都重新调用 → 又抢。
						// 两次都在"逻辑层"打补丁（删 autoFocus、加 searchFocusedOnceRef），
						// 但只要该 input **始终存在于 DOM 中**，任何新代码路径都能再次抢焦点。
						// 现在改为：searchOpen=false 时**不渲染 slot 与 input** → 元素不存在，物理上无法被聚焦。
						// 展开动画由 slot 的 width 过渡承担（挂载时从 0 → 100px）。
						searchOpen ? h("div", { key: "slot", className: "pm-search-slot open" },
							h("div", { className: "pm-search-inner" },
								h("div", { className: "pm-search-box" }, [
									h("input", {
										key: "i", value: q, placeholder: "搜索…",
										// 回调 ref **只记录元素**，绝不做 focus() 等副作用
					...
					// 记忆数量胶囊：**搜索展开时自动缩成只显示数字**（用户定案）。
					// 原因：面板内容区约 540px，而展开态需要 612px（差 72px）→ 胶囊会被顶出屏幕。
					// 缩成纯数字后释放约 48px（"记忆" 两字 + 内边距），配合搜索框 100px 即可容纳。
					// 始终保留 title 提示，鼠标悬停仍能看到"记忆 N"的完整含义。
					h("span", {
						className: "pm-stat" + (searchOpen ? " pm-stat-mini" : ""),
						title: "当前项目的记忆条数：" + memories.length,
					}, searchOpen ? h("b", null, memories.length) : ["记忆", h("b", { key: "n" }, memories.length)])),
