...
`;
		...
		function MemorySection() {
			...
			// 记忆搜索框的 DOM 引用：用于"仅展开时聚焦一次"（替代 autoFocus，见下方 useEffect）
			...
			// 记忆搜索框：**只在本框刚展开时聚焦一次**。
			// 不能用 input 的 autoFocus（该输入框始终在 DOM 中，React 每次重渲染都会抢焦点 →
			// 用户点目录输入框时焦点被夺走，字打进搜索框）。这里显式判断"刚打开"再聚焦。
			useEffect(() => {
				...
				const t = setTimeout(() => {
					...
					// 只有当焦点不在任何输入框时才抢（避免打断用户正在别处输入）
					const typingElsewhere = active && active !== el && /^(INPUT|TEXTAREA|SELECT)$/.test(active.tagName || "");
					if (!typingElsewhere) { try { el.focus(); const n = String(el.value || "").length; el.setSelectionRange(n, n) } catch (e) { /* 忽略 */ } }
				}, 60);
				return () => clearTimeout(t);
			}, [searchOpen]);
							...
							h("div", { className: "pm-search-inner" },
								h("div", { className: "pm-search-box" }, [
									h("input", {
										...
										// 回调 ref：同时记录到 searchInputRef（供"仅展开时聚焦一次"使用）与 focusEnd
										ref: (el) => { searchInputRef.current = el; focusEnd(el); },
