...
/* 搜索：默认收成图标按钮，点击展开输入框（用户需求） */
.pm-search-btn{display:inline-flex;align-items:center;justify-content:center;background:#1f1f1f;color:#b5b5b5;border:1px solid #2c2c2c;border-radius:20px;padding:5px 12px;cursor:pointer;flex:0 0 auto}
.pm-search-btn:hover{background:#262626}
.pm-search-btn.on{background:#3a3a3a;color:#f0f0f0;border-color:#3a3a3a}
.pm-search-inline{flex:1 1 90px;min-width:0;background:#1a1a1a;color:#e6e6e6;border:1px solid #333;border-radius:20px;padding:5px 12px;font-size:12px;outline:none;animation:pm-search-in .18s ease}
@keyframes pm-search-in{from{opacity:0;transform:scaleX(.6);transform-origin:left}to{opacity:1;transform:scaleX(1)}}
		...
		function MemorySection() {
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-bar" },
					...
					// 搜索：默认收成图标按钮（放在「已归档」右边），点一下向右展开输入框（用户需求）
					searchOpen
						? h("input", {
							className: "pm-search-inline", value: q, placeholder: "搜索标题、描述或正文…", autoFocus: true,
							onChange: (e) => setQ(e.target.value),
							onKeyDown: (e) => { if (e.key === "Escape") { if (!q) setSearchOpen(false); else setQ(""); } },
						})
						: h("button", {
							className: "pm-search-btn" + (q ? " on" : ""), onClick: () => setSearchOpen(true),
							title: q ? ("搜索中：" + q) : "搜索记忆",
						}, h("svg", {
