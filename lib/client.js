...
.pm-pick{display:inline-flex;align-items:center;flex:none}
...
`;
		...
		function MemorySection() {
			...
			// 「选择目录」按钮（用户需求）：调宿主弹系统文件夹对话框（桌面端=原生对话框）→
			// 选中即切换并加载该项目记忆；取消静默；本环境无原生能力时如实提示（退回手输/下拉）。
			const pickFolder = () => {
				setMsg("正在等待选择文件夹…");
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-field" },
					...
					// 「选择目录」：弹系统文件夹对话框（桌面端原生），选中即加载该目录的项目记忆
					h("button", { className: "pm-btn pm-pick", onClick: pickFolder, title: "从系统文件夹对话框里选一个项目目录" },
						h("svg", {
