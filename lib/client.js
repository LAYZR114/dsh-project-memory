...
`;
		...
		function MemorySection() {
			...
			const [sortDone, setSortDone] = useState(null); // 保存成功告知弹窗：{ label, count }（2 秒后自动关闭）
			...
			const load = useCallback((c, opts) => {
				...
				return fetchJson("/list", { method: "POST", body: JSON.stringify({ action: "list", cwd: c }) })
					.then((r) => { setMemories(r.ok ? (r.memories || []) : []); if (!keep) setMsg(r.ok ? "" : "加载失败: " + (r.error || "?")); })
			...
			// 保存成功告知弹窗：2 秒后自动关闭（用户要求的"自动存在两秒"）
			useEffect(() => {
				if (!sortDone) return undefined;
				const t = setTimeout(() => setSortDone(null), 2000);
				return () => clearTimeout(t);
			}, [sortDone]);
			...
			return h("div", { className: "pm" },
				...
				// 保存成功告知弹窗（2 秒后自动关闭；仅服务端回读校验通过时出现）
				sortDone ? h("div", { className: "pm-confirm-overlay" },
					h("div", { className: "pm-confirm" },
						...
						h("p", { className: "pm-cmsg" }, "已回读校验通过 · 本提示 2 秒后自动关闭"))) : null
			);
		}
