...
.pm-stat{height:36px;box-sizing:border-box;display:inline-flex;align-items:center;flex:0 0 auto;background:#1d1d1d;border:1px solid #2c2c2c;border-radius:999px;padding:0 12px;font-size:12px;color:#b5b5b5;white-space:nowrap;transition:margin .22s ease-out}
...
/* 只有输入框可压缩（flex:1 1 auto + min-width:0）；其余一律 flex:0 0 auto。
   依据：nowrap 行里未声明不可压缩的项会吸收全部压缩 → 控件被挤没（TruePPM #1008 同类修复）。 */
.pm-toolbar .pm-ac-wrap{flex:1 1 auto;min-width:110px}
...
.pm-input{flex:1 1 auto;min-width:0;height:36px;box-sizing:border-box;background:#1a1a1a;color:#e6e6e6;border:1px solid #333;border-radius:10px;padding:0 12px;font-size:13px;line-height:34px;outline:none}
...
/* 搜索：图标按钮常驻；展开时输入框**向右推开**（格子 0fr→1fr 过渡），右侧「记忆 N」自然让位。
   ① 用 grid-template-columns 而不是 width 做动画 → 内容不变形、不触发布局跳变（Animation Best Practices）；
   ② 时长 220ms + ease-out（微交互 100–200ms、显隐 200–300ms；绝不用 ease-in）；
   ③ 展开时按钮 = 框内左端图标位（同底色同边框）→ 视觉上仍是一个胶囊；
   ④ 全程**零遮挡**：不覆盖任何同行控件（用户定案：向右展开 + 胶囊让位动画）。 */
.pm-search{display:inline-flex;align-items:center;flex:0 0 auto;min-width:0}
.pm-search-btn{display:inline-flex;align-items:center;justify-content:center;height:36px;width:40px;flex:0 0 auto;background:#141414;color:#b5b5b5;border:1px solid #141414;border-radius:10px;padding:0;cursor:pointer;font-size:13px;box-sizing:border-box;transition:background .15s ease,color .15s ease}
.pm-search-btn:hover{background:#1f1f1f;color:#e6e6e6}
.pm-search-btn.on{background:#141414;color:#8f8f8f}
.pm-search-slot{display:grid;grid-template-columns:0fr;transition:grid-template-columns .22s ease-out}
.pm-search-slot.open{grid-template-columns:1fr}
.pm-search-inner{overflow:hidden;min-width:0}
.pm-search-box{display:inline-flex;align-items:center;gap:6px;height:36px;width:170px;max-width:34vw;background:#141414;border:1px solid #141414;border-left:none;border-radius:0 10px 10px 0;padding:0 10px 0 8px;box-sizing:border-box}
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-toolbar" },
					...
					h("div", { key: "search", className: "pm-search" }, [
						...
						// 展开的输入框放在**格子(slot)**里：0fr→1fr 过渡 → 向右推开、「记忆 N」让位（零遮挡）
						h("div", { key: "slot", className: "pm-search-slot" + (searchOpen ? " open" : "") },
							h("div", { className: "pm-search-inner" },
								h("div", { className: "pm-search-box" }, [
									h("input", {
										key: "i", value: q, placeholder: "搜索标题、描述或正文…", autoFocus: true, ref: focusEnd, tabIndex: searchOpen ? 0 : -1,
