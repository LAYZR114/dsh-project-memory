...
.pm-stats{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:0;align-items:center}
.pm-stat{height:36px;box-sizing:border-box;display:inline-flex;align-items:center;background:#1d1d1d;border:1px solid #2c2c2c;border-radius:999px;padding:0 12px;font-size:12px;color:#b5b5b5;white-space:nowrap;flex:0 0 auto}
...
/* —— 紧凑工具条：**单行不换行** —— 目录输入 + 保存 + 搜索 + 记忆胶囊（用户要求：全在同一行）——
   "最近：xxx" 不再挤在这一行（它会撑满导致折行，把搜索/胶囊挤到第二行 —— 用户实测），
   改到下面筛选行右侧显示。 */
.pm-toolbar{display:flex;align-items:center;gap:8px;flex-wrap:nowrap;margin-bottom:8px}
.pm-toolbar .pm-ac-wrap{flex:0 1 260px;min-width:140px}
.pm-top-actions{display:flex;gap:8px;flex:none;align-items:flex-start}
.pm-stat-recent{flex:0 1 auto;min-width:0;max-width:230px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block}
.pm-label{color:#c9c9c9;font-size:13px;white-space:nowrap;flex:0 0 auto}
.pm-input{flex:1;height:36px;box-sizing:border-box;background:#1a1a1a;color:#e6e6e6;border:1px solid #333;border-radius:10px;padding:0 12px;font-size:13px;line-height:34px;outline:none}
...
/* 搜索（用户要求：挪到「项目目录」那一行，与目录输入框同排 → 整页更整齐）：
   ① 默认只显示**图标按钮**，边框做成**深色（黑色系）**与搜索框同色系（不再白边突兀）；
   ② 点它 → 输入框**覆盖式滑出**（绝对定位，不参与 flex 布局 → 不挤压、不产生左右滚动条）；
   ③ 展开时按钮与覆盖层**同底色同黑边框**、框内自带同款放大镜 → 视觉上"一个胶囊展开"；
   ④ 空词再点 / 点外部 → 收起；
   ⑤ **高度与「项目目录」输入框（.pm-input，9px padding + 13px 字号）严格对齐** —— 用户要求。 */
.pm-search{position:relative;flex:0 0 auto;display:inline-flex;align-items:center}
.pm-search-btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;height:36px;min-width:40px;background:#141414;color:#b5b5b5;border:1px solid #141414;border-radius:10px;padding:0 12px;cursor:pointer;font-size:13px;box-sizing:border-box;position:relative;z-index:2;transition:background .15s ease,color .15s ease}
.pm-search-btn:hover{background:#1f1f1f;color:#e6e6e6}
.pm-search-btn.on{background:#141414;color:#8f8f8f;border-color:#141414;padding:0 11px}
.pm-search-box{position:absolute;left:0;top:50%;transform:translateY(-50%);z-index:3;display:inline-flex;align-items:center;gap:4px;height:36px;background:#141414;border:1px solid #141414;border-radius:10px;padding:0 10px 0 32px;box-sizing:border-box;box-shadow:0 8px 24px rgba(0,0,0,.45);animation:pm-search-in .18s ease}
.pm-search-box input{width:200px;max-width:38vw;background:transparent;border:none;color:#e6e6e6;font-size:13px;line-height:1;outline:none;padding:0}
.pm-search-box input::placeholder{color:#7a7a7a}
.pm-search-box .pm-search-lens{position:absolute;left:11px;top:50%;transform:translateY(-50%);color:#8f8f8f;pointer-events:none}
.pm-badge{flex:0 0 auto;font-size:11px;color:#8a8a8a;background:#2a2a2a;border-radius:999px;padding:1px 7px;max-width:96px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
@keyframes pm-search-in{from{opacity:0;transform:scaleX(.9);transform-origin:left center}to{opacity:1;transform:scaleX(1)}}
...
.pm-search{flex:1 1 120px;min-width:80px;background:#1a1a1a;color:#e6e6e6;border:1px solid #333;border-radius:10px;padding:9px 12px;font-size:13px;outline:none}
...
`;
		...
		function MemorySection() {
			...
			// 展开搜索后把光标放到**末尾**（否则一展开可能全选/顶到开头，打断继续编辑）
			const focusEnd = (el) => {
				if (!el) return;
				try { el.focus(); const n = String(el.value || "").length; el.setSelectionRange(n, n) } catch (e) { /* 老浏览器忽略 */ }
			};
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-toolbar" },
					...
					h("div", { className: "pm-ac-wrap" },
						...
						dirOpen ? (dirFiltered(cwd).length
							...
							: null),
					// 保存 + 「记忆 N」胶囊紧跟输入框（用户要求：两枚都往左移、与目录搜索框挨着）					h("button", { className: "pm-save", onClick: submitDir }, "保存"),
					// 搜索（用户要求：挪到「项目目录」这一行 → 整页更整齐）。图标按钮常驻、深色细边框与搜索框同色系；
					// 点它 → 输入框**覆盖式滑出**（绝对定位，不挤压布局/不产生横向滚动）；空词再点或点外部收起。
					h("div", { key: "search", className: "pm-search" }, [
						...
						searchOpen ? h("div", { key: "box", className: "pm-search-box" }, [
							// 覆盖层自带同款图标（与按钮同位同色）——视觉上就是"按钮变成了输入框"
							h("svg", {
								key: "lens2", className: "pm-search-lens", width: "13", height: "13", viewBox: "0 0 24 24",
								fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round",
							}, [h("circle", { key: "c", cx: "11", cy: "11", r: "8" }), h("line", { key: "l", x1: "21", y1: "21", x2: "16.65", y2: "16.65" })]),
							h("input", {
								key: "i", value: q, placeholder: "搜索标题、描述或正文…", autoFocus: true, ref: focusEnd,
								onChange: (e) => setQ(e.target.value),
								onBlur: () => setTimeout(() => { if (!q) setSearchOpen(false) }, 150), // 点外部收起（有词则保留）
				...
				h("div", { className: "pm-bar" },
					...
					// 「最近：xxx」放在**筛选行**（不再挤目录行 —— 它会撑满导致折行，把搜索/胶囊挤下去）
					(function () {
						const recent = [...memories].sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt))[0];
						return recent ? h("span", { className: "pm-stat pm-stat-recent", title: recent.title }, "最近：", h("span", { className: "pm-recent" }, recent.title)) : null;
					})(),
					// 排序**常驻最右**（不再为搜索让位 —— 让位会把整行撑宽、出现左右滚动条）
					(filter !== "archived") ? h("span", { style: { marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: "8px", flex: "0 0 auto" } },
