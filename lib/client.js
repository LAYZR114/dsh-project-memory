...
.pm-stat{height:36px;box-sizing:border-box;display:inline-flex;align-items:center;flex:0 0 auto;background:#1d1d1d;border:1px solid #2c2c2c;border-radius:999px;padding:0 12px;font-size:12px;color:#b5b5b5;white-space:nowrap;flex:0 0 auto}
...
/* —— 目录行（用户定案：**一行、全部紧挨、行尾不留空**）——
   项目目录 [输入框] [保存] [🔍] (记忆 63)
   关键：① 不换行；② **除输入框外一律 flex:0 0 auto（不许被压没）**；
        ③ 输入框固定宽度（不用 flex:1 抢空间）→ 后面的按钮自然紧贴它，也不会把「保存」压消失。 */
.pm-toolbar{display:flex;align-items:center;gap:8px;flex-wrap:nowrap;margin-bottom:8px}
.pm-toolbar .pm-ac-wrap{flex:0 0 210px}
.pm-top-actions{display:flex;gap:8px;flex:none;align-items:flex-start}
/* 「最近：xxx」单独一行，放在筛选胶囊**上方**（用户要求：给它足够宽度，不被压缩） */
.pm-recent-row{display:flex;align-items:center;gap:8px;margin-bottom:8px}
.pm-recent-row .pm-stat-recent{flex:1 1 auto;min-width:0;max-width:none;color:#c9c9c9}
.pm-stat-recent{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block}
...
.pm-search{position:relative;flex:0 0 auto;display:inline-flex;align-items:center;min-width:0}
.pm-search-btn{display:inline-flex;align-items:center;justify-content:center;gap:0;height:36px;width:40px;flex:0 0 auto;background:#141414;color:#b5b5b5;border:1px solid #141414;border-radius:10px;padding:0 12px;cursor:pointer;font-size:13px;box-sizing:border-box;position:relative;z-index:2;transition:background .15s ease,color .15s ease}
