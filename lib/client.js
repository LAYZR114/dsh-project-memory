window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const inject = ["slots"];
		// 客户端可用的"工作区/目录"服务（软依赖，apply 时 ctx.get('uiWorkspace') 取）：
		// 它的 pickDirectory() 正是 DSH 应用自己"选文件夹"的那个对话框（含面包屑+新建文件夹），
		// 比宿主端的原生 pick 更普适（桌面端实现的是 browse 式，没有系统原生对话框）。
		let wsPicker;
...
`;
		...
		function MemorySection() {
			...
			const pickFolder = () => {
				...
				// ① 应用自带的选择器（最普适）
				if (wsPicker && typeof wsPicker.pickDirectory === "function") {
					setMsg("正在等待选择文件夹…");
					Promise.resolve()
						.then(() => wsPicker.pickDirectory())
			...
			// 退回路径：宿主 action:'pickdir'（系统原生对话框；没有原生能力时如实提示）
				...
				fetchJson("/pickdir", { method: "POST", body: JSON.stringify({ action: "pickdir" }) })
					.then((r) => {
						if (!r || !r.ok) {
							...
							if (err === "browse-only") { setMsg("本环境没有系统文件夹对话框，请直接输入目录或从下拉里选。"); return; }
							setMsg("选择目录失败：" + err);
							return;
						}
						if (!finish(r.cwd)) setMsg("");
					})
		...
		function apply(ctx) {
			// 软依赖：客户端"工作区/目录"服务（提供 pickDirectory()，即应用自带的选文件夹对话框）。
			// 用 ctx.get 探测而非 inject：没有它插件照常工作（退回宿主 pickdir / 手输）。
