...
`;
		...
		function MemorySection() {
			...
			const filtered = useMemo(() => {
				...
				// 排序（default=手动不重排；其余按规则）
				if (sort === "newest") return [...base].sort((a, b) => new Date(b.updatedAt || 0) - new Date(a.updatedAt || 0));
				if (sort === "important") return [...base].sort((a, b) => (b.heat || 0) - (a.heat || 0) || (b.proofCount || 0) - (a.proofCount || 0));
				if (sort === "inject") return [...base].sort((a, b) => (b.sources ? b.sources.length : 0) - (a.sources ? a.sources.length : 0));
				return base;
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-bar" },
					...
					(filter !== "archived") ? h("select", { className: "pm-select", value: sort, onChange: (e) => setSort(e.target.value), style: { marginLeft: "auto" } },
						h("option", { value: "default" }, "排序：默认"),
						h("option", { value: "newest" }, "从新到旧"),
						h("option", { value: "important" }, "重要等级高→低"),
						h("option", { value: "inject" }, "自动注入次数多→少")) : null),
				h("ul", { className: "pm-list", ref: listRef }, listChildren),
				msg ? h("div", { className: "pm-msg" }, msg) : null,
