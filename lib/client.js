				...
				fetchJson("/pickdir", { method: "POST", body: JSON.stringify({ action: "pickdir" }) })
					.then((r) => {
						...
						if (err === "browse-only" || err === "picker unavailable") {
							// 没有系统对话框 → 先**探测**浏览能力：能用才弹自家浏览框；不能用就直接告诉用户怎么手输
							//（2026-09-11 用户反馈：探测失败还弹一个空对话框，又慢又没用）
							const start = String(cwd || "").trim();
							fetchJson("/browse", { method: "POST", body: JSON.stringify({ action: "browse", path: start || undefined }) })
								.then((b) => {
									if (b && b.ok && b.listing) { setMsg(""); setBrowse({ loading: false, listing: b.listing, error: "" }); return; }
									setMsg("这个环境没有可用的目录选择器，请直接把路径粘进输入框，或从下拉里选一个现有项目。");
								})
								.catch(() => setMsg("这个环境没有可用的目录选择器，请直接把路径粘进输入框，或从下拉里选一个现有项目。"));
							return;
						}
						setMsg("选择目录失败：" + err);
					})
