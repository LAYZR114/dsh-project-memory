...
.pm-stat{background:#1d1d1d;border:1px solid #2c2c2c;border-radius:999px;padding:7px 12px;font-size:12px;color:#b5b5b5;white-space:nowrap;flex:0 0 auto}
...
/* —— 紧凑工具条：目录 + 保存 + 统计同一行（输入框**不抢占宽度** → 保存/胶囊紧贴它）—— */
.pm-toolbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px}
.pm-toolbar .pm-ac-wrap{flex:0 1 300px;min-width:170px}
.pm-top-actions{display:flex;gap:8px;flex:none;align-items:flex-start}
.pm-stat-recent{max-width:330px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block}
...
.pm-save{background:#3a3a3a;color:#f0f0f0;border:none;border-radius:10px;padding:8px 18px;font-size:13px;cursor:pointer;white-space:nowrap;flex:0 0 auto}
...
/* 搜索：照抄成熟做法（Codrops《Expanding Search Bar》思路 https://tympanus.net/codrops/2013/06/26/expanding-search-bar-deconstructed/ ）：
   ① 默认只显示**图标按钮**；② 点它，输入框**从图标处滑出**；③ 输入框是**绝对定位覆盖层**（overlay）——
      不参与 flex 布局，所以既不挤压筛选胶囊、也不产生左右滚动条；④ 宽度够宽（300px）看得清；
   ⑤ 再点空词的按钮、或点外部 → 收起；⑥ **展开时按钮与输入框同底色同边框、无缝拼成一个胶囊**
      （展开后原来的按钮只作为框内左端的图标位，不再是一块不同色的按钮——用户反馈"颜色突兀"）。 */
.pm-search{position:relative;flex:0 0 auto}
.pm-search-btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;background:#1a1a1a;color:#b5b5b5;border:1px solid #2c2c2c;border-radius:20px;padding:5px 12px;cursor:pointer;height:30px;box-sizing:border-box;position:relative;z-index:2}
.pm-search-btn:hover{background:#262626}
/* 展开态：按钮保留但**只当图标位**——与覆盖层同色、边框让给覆盖层，视觉上合为一个控件 */
.pm-search-btn.on{background:#1a1a1a;color:#8f8f8f;border-color:#4a4a4a;padding:5px 10px}
.pm-search-box{position:absolute;left:0;top:0;z-index:3;display:inline-flex;align-items:center;gap:4px;background:#1a1a1a;border:1px solid #4a4a4a;border-radius:20px;padding:0 10px 0 34px;height:30px;box-sizing:border-box;box-shadow:0 8px 24px rgba(0,0,0,.45);animation:pm-search-in .18s ease}
.pm-search-box input{width:268px;max-width:48vw;background:transparent;border:none;color:#e6e6e6;font-size:12px;outline:none;padding:0}
		...
		function MemorySection() {
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-toolbar" },
					...
					h("div", { className: "pm-ac-wrap" },
						...
						dirOpen ? (dirFiltered(cwd).length
							...
							: null),
					// 保存 + 「记忆 N」胶囊紧跟输入框（用户要求：两枚都往左移、与目录搜索框挨着）
				...
				h("div", { className: "pm-bar" },
					...
					// 搜索（照抄 Codrops 展开式搜索做法）：图标按钮常驻；点它 → 输入框**覆盖式滑出**
					// （绝对定位，不挤压胶囊/排序、不产生横向滚动）；空词再点或点外部收起。
					// 展开时**按钮只当框内左端的图标位**（同底色同边框，无缝拼成一个胶囊），覆盖层**不越过排序**。
					h("div", { key: "search", className: "pm-search" }, [
						...
						searchOpen ? h("div", { key: "box", className: "pm-search-box" }, [
							// 覆盖层自带一个图标（与下面按钮同位同色）——视觉上就是"按钮变成了输入框"
							h("svg", {
