...
/* 搜索：**一体式胶囊** —— 图标常驻在框内左侧，点击框（或图标）即聚焦展开；展开时保持显示并与输入框融合 */
.pm-search-wrap{flex:1 1 160px;min-width:120px;display:flex;align-items:center;gap:6px;background:#1a1a1a;border:1px solid #2c2c2c;border-radius:20px;padding:4px 12px;cursor:pointer;animation:pm-search-in .18s ease}
.pm-search-wrap:focus-within{border-color:#555;cursor:text}
.pm-search-wrap svg{flex:0 0 auto;color:#8f8f8f}
.pm-search-wrap input{flex:1;min-width:0;background:transparent;border:none;color:#e6e6e6;font-size:12px;outline:none;padding:2px 0}
.pm-search-wrap input::placeholder{color:#7a7a7a}
...
@keyframes pm-search-in{from{opacity:0;transform:scaleX(.92);transform-origin:left}to{opacity:1;transform:scaleX(1)}}
		...
		function MemorySection() {
			...
			return h("div", { className: "pm" },
				...
				h("div", { className: "pm-bar" },
					...
					// 搜索：**一体式胶囊** —— 放大镜图标常驻框内左侧，点击框即聚焦展开；展开后与输入框融合（用户反馈）
					(function () {
						const lens = h("svg", {
						...
						// 展开后把光标放到**末尾**（否则一展开可能全选/顶到开头，打断继续编辑）
						const focusEnd = (el) => {
							if (!el) return;
						...
						if (!searchOpen) {
							return h("button", {
								key: "search", className: "pm-search-wrap" + (q ? " on" : ""), onClick: () => setSearchOpen(true),
								title: q ? ("搜索中：" + q) : "搜索记忆",
							}, [lens, h("input", {
								key: "i", readOnly: true, value: q, placeholder: "搜索",
								style: { cursor: "pointer" }, tabIndex: -1,
							})]);
						}
						return h("div", { key: "search", className: "pm-search-wrap" }, [
							lens,
							h("input", {
					...
					// 搜索展开时**占用排序的位置**（融合一颗胶囊），收起时排序回来
					(filter !== "archived" && !(searchOpen && q)) ? h("span", { style: { marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: "8px", flex: "0 0 auto" } },
