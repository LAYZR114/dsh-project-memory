...
> **版本**：0.1.4 · **许可证**：BSD-3-Clause · **类型**：DeepSeek Harness（DSH）插件 · **适配核心**：DSH **v0.1.5-rc.1 及更新**（web/desktop profile 通用；桌面端 2.0.7+）
> **版本线**：🚀 **新核线 0.1.4**（跟随 DSH 最新核心；功能与自用稳定线 0.1.3 对等，另含核心适配。两线同一包名、一个 profile 只装一条；**只有出现较大优化或新功能时才升 0.2.0**。详见 [`VERSION-LINES.md`](./VERSION-LINES.md)）
> **0.1.4**：含设置页「**保存排序**」按钮（选任一自动排序后一键固化为「默认（手动）顺序」，默认模式不显示该按钮，保存后 10 秒内可撤销）。
