...
> **0.1.3 布局紧凑化（2026-09-11 用户反馈："大空位多、按钮要重排"）**：设置页重排为**三行结构**——① 标题 + 右侧成组的「导出 / 导入 / 新增记忆」；② **一条工具条**：`项目目录 [输入框] [保存] · 记忆 N · 最近：xxx`（统计不再单独占一行，"最近"单行省略不换行）；③ **一条操作条**：`全部/活跃/待审/已归档 + 搜索 + 排序（+保存排序）` 同排不换行。间距整体收紧（顶部 16→8px、列表 10→8px、分组头 padding 10/14→8/12px）。**注**：「选择目录」按钮因本环境不提供可用的系统文件夹对话框，已按用户要求**整体撤销**（客户端/宿主/API 均无残留）——切目录用输入框回车或下拉选择。
