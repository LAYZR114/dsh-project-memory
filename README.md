...
> **0.1.3 选择目录按钮（2026-09-11 用户需求）**：目录输入框右侧新增「**选择目录**」（Feather 线性文件夹图标，**与「保存」同款同尺寸**）。点击后**四级优先**取目录，目标就是像导入按钮那样弹**系统的文件夹对话框**：① **宿主 `directoryPickerController.pick()`** —— 驱动 DSH 应用自带的系统"选择文件夹"对话框（桌面端实测可用、最快）；② **客户端 `uiWorkspace.pickDirectory()`**（惰性获取，服务晚注册也能拿到）；③ 宿主 `ctx.directoryPicker` 原生 `pick()`；④ 都没有 → `action:'browse'` **探测**官方 browse 能力：能用才弹插件自带的浏览对话框，**不能用就直接提示手输路径**（不再弹空对话框）。选中即切到该项目并加载其记忆、记入最近使用；用户取消一律静默。
