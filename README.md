...
> **0.1.3 选择目录按钮（2026-09-11 用户需求）**：目录输入框右侧新增「**选择目录**」（Feather 线性文件夹图标，**与「保存」同款同尺寸**）。点击后**三级优先**取目录：① **客户端服务 `uiWorkspace.pickDirectory()`**（**惰性获取**——服务在 apply 之后注册也能拿到）即 DSH 应用自带"选文件夹"对话框；② 宿主 `ctx.directoryPicker` 的原生 `pick()`（有系统对话框的环境）；③ 都没有 → **弹出插件自带的文件夹浏览对话框**（走官方 browse 能力列目录：面包屑、点进子目录、"选这个文件夹"）。选中即切到该项目并加载其记忆、记入最近使用；用户取消一律静默。
