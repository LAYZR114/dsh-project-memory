...
> **0.1.3 选择目录按钮（2026-09-11 用户需求）**：目录输入框右侧新增「**选择目录**」（Feather 线性文件夹图标，**与「保存」同款同尺寸**）。点击后按优先级取目录：① **客户端官方服务 `uiWorkspace.pickDirectory()`** —— 就是 DSH 应用自己"选文件夹"的那个对话框（桌面端可用，带面包屑/新建文件夹）；② 不可用时退回**宿主** `ctx.directoryPicker` 的原生 `pick()`（系统文件夹对话框）；③ 都没有则**如实提示**改用手输/下拉。选中即切到该项目并加载其记忆、记入最近使用；用户取消一律静默。
