...
> **0.1.3 选择目录按钮（2026-09-11 用户需求）**：目录输入框右侧新增「**选择目录**」（Feather 线性文件夹图标）——点它调官方宿主 `ctx.directoryPicker`（桌面端=**系统原生文件夹对话框**），选中即切到该项目并加载其记忆、记入最近使用；用户取消则静默；若本环境只有浏览能力（无原生对话框）会**如实提示**并建议改用手输/下拉，绝不假装成功。
