...
> **版本**：0.1.5 · **许可证**：BSD-3-Clause · **类型**：DeepSeek Harness（DSH）插件 · **适配核心**：DSH **v0.1.5-rc.1 及更新**（web/desktop profile 通用；桌面端 2.0.7+）
> **版本线**：🚀 **新核线 0.1.5**（跟随 DSH 最新核心；功能与自用稳定线 0.1.4 对等，另含核心适配。两线同一包名、一个 profile 只装一条；**只有出现较大优化或新功能时才升 0.2.0**。详见 [`VERSION-LINES.md`](./VERSION-LINES.md)）
> **0.1.5 修复（重要）**：手动顺序以前**根本存不住** —— `readDoc` 每次读盘都按"最新"重排，写进去的手动/保存顺序一读就被冲掉（拖动排序也一直因此无效）。现读取新增 `keepOrder`：设置页 `/list` 返回**文件真实顺序**；`/save` 写盘后**回读校验**并返回 `verified`，客户端据此才报成功（不一致就提示"未生效"并重载磁盘实际顺序）。
> **0.1.5 交互**：点「保存排序」先弹**二次确认**弹窗 → 确认后写盘 → 回读校验通过 → 弹**成功告知弹窗（2 秒自动关闭）**；默认（手动）模式下按钮不显示；保存后 10 秒内仍可撤销。
