...
> **0.1.3 布局紧凑化·第二轮（2026-09-11 用户微调）**：① 四个筛选胶囊（全部/活跃/待审/已归档）**文字强制单行横排**（`nowrap + flex:0 0 auto + word-break:keep-all`，不再被挤成竖排）；② **保存按钮 + 「记忆 N」胶囊左移**紧贴目录输入框（去掉 `margin-left:auto`），"最近：…"跟在它们右边；③ 搜索框改为**默认收起的图标按钮**（放在「已归档」右侧，Feather 放大镜线稿），**点击向右展开**输入框（带 `pm-search-in` 横向展开动画），Esc 先清词、空词时收起；④ 排序下拉**不减文案**（仍为"排序：默认 / 排序：从新到旧 / 排序：重要等级高→低 / 排序：自动注入多→少"，改用全角冒号），通过 `max-width:190px` 压窄宽度。
